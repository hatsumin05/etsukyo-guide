...
トビタテ・AFS・AIG高校生外交官・YFU・UWC など、毎年募集がある定番プログラムは
`data/manual-candidates.json` に登録済みです(id が `manual-` で始まるもの)。

これらは**年に一度、募集時期が来たら締切を更新する**必要があります。次の点に注意してください。

- 次回の募集要項がまだ公表されていない場合は `applyEnd` を `null` のままにし、
  `deadline` 欄に「いつ公開されるか」を書く(例: 「第12期は2026年秋ごろ公開予定(要公式確認)」)
- **公式サイトで確認できない締切を推測で書かないでください。** 生徒が締切を逃す原因になります
- `applyEnd` が `null` のものは「応募受付中」一覧には出ませんが、「タグで探す」には出るので、
  存在自体は生徒に伝わります
